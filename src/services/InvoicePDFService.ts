...
const TABLE_COLUMNS = [28, 176, 58, 42, 66, 64, 77];
...
export class InvoicePDFService {
  ...
  private static drawHeader(doc: PDFKit.PDFDocument, invoice: CompleteInvoice): void {
    const title = invoice.invoice_type.replace(/_/g, ' ').toUpperCase();
    const balanceDue = Number(invoice.balance_amount || 0);
    const paidInFull = balanceDue <= 0;

    doc.roundedRect(MARGIN, MARGIN - 12, CONTENT_WIDTH, 80, 6).fill('#eff6ff');
    doc
      .font('Helvetica-Bold')
      .fontSize(22)
      .fillColor('#1e3a8a')
      .text(invoice.business_name || 'Hall Sync', MARGIN, MARGIN, {
        width: 300,
      });
    doc
      .font('Helvetica-Bold')
      .fontSize(18)
      .fillColor('#111827')
      .text(title, PAGE_WIDTH - MARGIN - 190, MARGIN, {
        width: 190,
        align: 'right',
      });
    doc
      .roundedRect(PAGE_WIDTH - MARGIN - 116, MARGIN + 25, 116, 18, 9)
      .fill(paidInFull ? '#dcfce7' : '#ffedd5');
    doc
      .font('Helvetica-Bold')
      .fontSize(8)
      .fillColor(paidInFull ? '#166534' : '#9a3412')
      .text(paidInFull ? 'PAID IN FULL' : 'BALANCE DUE', PAGE_WIDTH - MARGIN - 108, MARGIN + 31, {
        width: 100,
        align: 'center',
      });
    doc
      ...
      .fontSize(9)
      .fillColor('#4b5563')
      .text(invoice.business_gstin ? `GSTIN: ${invoice.business_gstin}` : 'GSTIN: Not provided', MARGIN, 70)
      .text(`Status: ${String(invoice.status).replace(/_/g, ' ').toUpperCase()}`, PAGE_WIDTH - MARGIN - 190, 70, {
        width: 190,
        align: 'right',
      });
    doc
      .moveTo(MARGIN, 91)
      .lineTo(PAGE_WIDTH - MARGIN, 91)
      .lineWidth(1.5)
      .strokeColor('#2563eb')
      .stroke();
    ...
    if (invoice.booking_id) {
      this.labelValue(
        doc,
        'Booking Ref.',
        `BK-${String(invoice.booking_id).padStart(6, '0')}`,
        MARGIN,
        detailsY + 36,
        225
      );
  ...
  private static drawTableHeader(doc: PDFKit.PDFDocument): void {
    ...
    const y = doc.y;
    doc.rect(MARGIN, y, CONTENT_WIDTH, 24).fill('#1e3a8a');
    this.drawTableRow(
      doc,
  ...
  private static drawTotals(doc: PDFKit.PDFDocument, invoice: CompleteInvoice): void {
    ...
    rows.forEach(([label, value, emphasized]) => {
      const y = doc.y;
      if (emphasized) {
        doc.rect(x, y - 2, 235, 18).fill(label === 'Grand Total' ? '#dbeafe' : '#fef3c7');
      }
      doc
        .font(emphasized ? 'Helvetica-Bold' : 'Helvetica')
        .fontSize(emphasized ? 9.5 : 8.5)
        .fillColor('#111827')
        .text(label, x + 8, y + 2, { width: 105 })
        .text(money(value), x + 118, y + 2, { width: 109, align: 'right' });
